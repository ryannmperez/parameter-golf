...
# Try flash_attn, fall back to PyTorch SDPA
try:
    from flash_attn_interface import flash_attn_func as _flash_attn_func
    _ATTN_BACKEND = "flash3"
except ImportError:
    try:
        from flash_attn import flash_attn_func as _flash_attn_func
        _ATTN_BACKEND = "flash2"
    except ImportError:
...
class Hyperparameters:
    ...
    warmup_steps = int(os.environ.get("WARMUP_STEPS", 0))  # disabled: wastes ~37s (12% of budget) since state is reset
...
def eval_val(
    ...
    is_boundary_token_lut: Tensor, eval_seq_len: int | None = None,
) -> tuple[float, float]:
...
def _attn_forward(q: Tensor, k: Tensor, v: Tensor, num_heads: int, num_kv_heads: int) -> Tensor:
    """Attention dispatch: flash_attn (BTHD) or SDPA (BHTD)."""
    if _flash_attn_func is not None:
        return _flash_attn_func(q, k, v, causal=True)
    # Fall back to SDPA: convert BTHD -> BHTD
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        y = _attn_forward(q, k, v, self.num_heads, self.num_kv_heads)
        if self.use_xsa:
...
def main() -> None:
    ...
    log0("=" * 100, console=False)
    log0(f"attn_backend:{_ATTN_BACKEND}")
    ...
    CastedLinear._qat_enabled = False  # Late QAT enabled during warmdown
    base_model = GPT(
    ...
    # Optimizer split: banks -> Muon, embeddings -> Adam, scalars -> Adam
    ...
    log0(f"train_batch_tokens:{args.train_batch_tokens} train_seq_len:{args.train_seq_len} iterations:{args.iterations} max_wallclock_seconds:{args.max_wallclock_seconds:.3f}")
    ...
    log0(f"peak memory allocated: {peak_vram_mb} MiB reserved: {torch.cuda.max_memory_reserved() // 1024 // 1024} MiB")
    ...
    final_bpb = sw_val_bpb if (args.eval_stride > 0 and args.eval_stride < sw_seq_len) else q_val_bpb
    compressed_size = os.path.getsize("final_model.int8.ptz") if os.path.exists("final_model.int8.ptz") else 0
    log0(f"val_bpb:              {final_bpb:.6f}")
